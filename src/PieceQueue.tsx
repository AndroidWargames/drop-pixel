...
const pieceGrid = (pieces: Piece[]): BoardData => {
  ...
  if (size == 0) { return [[]] as BoardData }
  let data = Array.from({length: size}, () => Array.from({length: 3}, () => black))
  let maxY = 0
  pieces.forEach((piece) => {
    console.log({piece, data})
    piece.chonks.forEach((chonk) => { data[chonk.y + maxY][chonk.x] = chonk.color })
...
const PieceDisplay = ({pieces}: {pieces: Piece[]}) => (
  <div style={style} className="Board">
    {pieceGrid(pieces).map((row, i) => <BlockRow key={i} y={i} rowColors={row}/>)}
  </div>
)
